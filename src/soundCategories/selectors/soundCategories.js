...
export const getCategoriesWithStatus = createSelector(
  ...
  (categories, sounds) =>
    categories.map(category => ({
      ...category,
      playing: category.sounds.some(soundUuid =>
        sounds.find(sound => sound.cachePath === soundUuid && sound.playing)
      )
    }))
)

export const getCategoryListUuids = createSelector(categoryStore.getAll, list =>
  list.map(category => category.uuid)
)
...
export const getCategoryByName = createSelector(
  (state, name) =>
    categoryStore.getAll(state).find(category => category.name === name),
  category => category
)
...
export const getCategorySounds = createSelector(
  ...
  (category, soundList) =>
    soundList.filter(sound => category.sounds.indexOf(sound.cachePath) !== -1)
)
...
export const getCategoryPlayingStatus = createSelector(
  ...
  (category, allSounds) =>
    allSounds
      .filter(sound => category.sounds.indexOf(sound.cachePath) !== -1)
...
export const getAllUnusedSoundsByTag = createSelector(
  ...
  (sounds, categories) =>
    sounds.filter(
      sound =>
        !categories.find(
          category => category.sounds.indexOf(sound.cachePath) !== -1
        )
    )
)
...
export const isSoundUsed = createSelector(
  ...
  (categories, soundUuid) =>
    categories.some(category => category.sounds.indexOf(soundUuid) !== -1)
)
